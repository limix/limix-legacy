...
def define_gp(Y, Xr, Sg, Ug, type):
    P = Y.shape[1]
    _A = sp.eye(P)
    if type in 'rank1':             _Cr = limix_legacy.core.covar.LowRankCov(P,1)
    elif type=='block':             _Cr = limix_legacy.core.covar.FixedCov(sp.ones((P,P)))
    elif type=='full':              _Cr = limix_legacy.core.covar.FreeFormCov(P)
    elif type=='null':              pass
    else:                           print('poppo')
    _Cn = limix_legacy.core.covar.FreeFormCov(P)
    _Cg = limix_legacy.core.covar.FreeFormCov(P)
